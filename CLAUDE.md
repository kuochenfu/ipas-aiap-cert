...
- **`session.bank`（`"main" | "practice"`）決定刷題的題庫來源**：`"main"` 讀 `getQuestions`（原題庫），`"practice"` 讀 `getPracticeQuestions`（`src/data/practice/`，依評鑑內容節點分類，目前七科各 100 題）。`drillProgressKey()` 依此在 key 後綴 `:practice`，讓兩者的進度分開存在同一個 `ipas-aiap-drill-progress` map 裡。改刷題相關邏輯（進度還原、篩選、就地更新等）時，兩種來源都要確認成立，不能只測 `bank: "main"` 的路徑。新題庫的節點題數與配額由 `src/domain/assessmentTopics.ts` 定義，改配額要同步改 `tests/practiceBank.test.ts`。

- **七科的新題庫都帶命題後設資料 `meta`（`QuestionMeta`）**：認知層級、題型原型、干擾類型、跨節點、判斷分界（`decisionBoundary`）。契約在 `tests/questionBankMeta.test.ts`，分兩層：**結構契約**（每題 meta 完整、干擾類型鍵＝選項解析鍵、判斷分界非空、詳解指名的選項字母須指向正解）適用全七科；**分布契約**（認知層級與難度區間、Direct Concept ≤15%、跨節點 ≥20%、字母均衡）只適用 AIoT 兩科——它們是照規格從零命製的，五科則是先寫好題目、事後補標，實測偏重記憶與理解，改以基線快照守住而非假裝符合規格。要讓五科的分布往上走，正確做法是**改寫題目**，不是改標籤。`renderDecisionBoundary` 現在七科都會輸出。
