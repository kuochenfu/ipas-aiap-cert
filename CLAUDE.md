...
- **只有 AIoT 兩科的新題庫帶命題後設資料 `meta`（`QuestionMeta`）**：認知層級、題型原型、干擾類型、跨節點、判斷分界（`decisionBoundary`）。契約在 `tests/aiotQuestionBank.test.ts`（節點配額、認知層級與難度分布區間、純記憶型上限、跨節點下限、字母均衡）。五科的新題庫沒有 `meta`，`renderDecisionBoundary` 因此整段不輸出，不會留空殼。
