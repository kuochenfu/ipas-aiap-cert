...
export const generated: Question[] = [
  ...
  },
  {
    ...
    choices: [
      { id: "A", text: "生成器（Generator）負責產生假樣本，判別器（Discriminator）負責分辨真假，兩者對抗訓練" },
      { id: "B", text: "兩個網路皆只負責生成樣本，彼此不互相評估" },
      { id: "C", text: "GAN 只能用於監督式分類，無法生成新資料" },
      { id: "D", text: "生成器負責分類，判別器負責迴歸預測" },
    ],
    answer: "A",
    explanation:
      "GAN 中生成器產生擬真的假樣本，判別器嘗試分辨真假，兩者在對抗中互相提升（A）。並非皆只生成且不互評（B）；GAN 的目的正是生成新資料（C 錯）；生成器與判別器的職責與 D 描述相反。",
    topic: "鑑別式 AI 與生成式 AI 概念",
    difficulty: "難",
    source: "generated",
  },
  ...
  },
  {
    ...
    choices: [
      { id: "A", text: "將不同尺度的特徵調整到可比較的範圍，避免大尺度特徵主導模型" },
      { id: "B", text: "把連續數值直接轉換為類別標籤" },
      { id: "C", text: "刪除資料中所有的離群值" },
      { id: "D", text: "為缺失值自動填補合理數值" },
    ],
    answer: "A",
    explanation:
      "標準化將特徵轉為相近尺度（如均值 0、標準差 1），避免量級較大的特徵在距離或梯度計算中主導模型（A）。它不做類別轉換（B）、不直接刪除離群值（C），也非缺失值填補（D）。",
    topic: "資料處理與分析概念",
    difficulty: "易",
    source: "generated",
  },
  ...
  },
  {
    ...
    choices: [
      { id: "A", text: "模型產生看似流暢合理、但與事實不符或無依據的內容" },
      { id: "B", text: "模型因記憶體不足而完全停止回應" },
      { id: "C", text: "模型只會逐字重複使用者輸入的內容" },
      { id: "D", text: "模型能完美無誤地回答所有問題" },
    ],
    answer: "A",
    explanation:
      "幻覺指 LLM 生成語句通順但與事實不符或缺乏依據的內容（A）。它與記憶體不足當機（B）、單純複述輸入（C）無關，更不代表模型全知全能（D）；理解幻覺有助於導入驗證與 RAG 等緩解措施。",
    topic: "鑑別式 AI 與生成式 AI 概念",
    difficulty: "易",
    source: "generated",
  },
