...
export const generated: Question[] = [
  ...
  },
  {
    ...
    choiceExplanations: {
      ...
      D: "商品推薦影響的是購物選擇，一般被歸為商業性的低風險應用；被列為不可接受風險的是危及基本權利的用途。",
    },
  ...
  },
  {
    ...
    choiceExplanations: {
      A: "題幹已說模型偶有事實錯誤與用詞偏差，全自動發布等於把這些瑕疵原封不動送到公眾面前，而對外正式公告一旦發出就難以收回。",
      B: "改回純手寫確實能避開模型的錯誤，但也放棄了以 AI 產草稿的效率；題幹問的是人機協作要怎麼定位，而不是要不要用 AI。",
      C: "互審的兩方共用相近的訓練來源與弱點，對事實錯誤未必查得出來，且最後仍沒有任何人為這份對外公告負責把關。",
    },
  ...
  },
  {
    ...
    choiceExplanations: {
      B: "訂單狀態隨時在變，截圖只是某一刻的靜態畫面，貼進提示詞後既反映不了最新狀態，筆數一多也無從維護。",
      C: "由客服人員逐筆口述等於把查詢工作退回人力，可承受的查詢量與回應速度都卡在人手上，違背整合進系統的目的。",
      D: "題幹要的是把生成式 AI 整合進既有客服系統，停用系統改全人工等於取消了這個要解的問題本身。",
    },
    topic: "L12202 如何善用生成式 AI 工具",
    difficulty: "中",
    source: "generated",
  },
  {
    ...
    choiceExplanations: {
      A: "全面禁止確實能避免權限與品質失控，但也一併消滅了讓業務單位自助解決問題的效益，還可能把建置行為推到看不見的地方；治理要的是可控的開放。",
      C: "讓每位使用者自行決定資料存取權限又不留紀錄，正是題幹所述失控的來源本身，事後連追查都無從下手。",
      D: "改用組合語言重寫不會建立起任何審核或權限規範，反而讓原本能自助維護應用的業務人員完全接不了手。",
    },
    topic: "L12101 No Code / Low Code 的基本概念",
    difficulty: "中",
    source: "generated",
  },
  {
    ...
    choiceExplanations: {
      A: "試辦的用意是先在小範圍驗證可行性與效益；一開始就全公司鋪開，等問題浮現時影響範圍已經收不回來。",
      B: "不界定範圍就沒有可對照的驗收基準，成效好壞無從判定，資源也會被分散到彼此無關的功能上。",
      C: "採用哪個模型是達成目標的手段而非目標本身；模型換得再新，客服的首次回覆時間與品質有沒有改善仍然沒有被衡量。",
    },
    topic: "L12302 生成式 AI 導入規劃",
    difficulty: "中",
    source: "generated",
  },
  {
    ...
    choiceExplanations: {
      B: "圖片去背處理的是前景與背景分離，輸入與輸出都是影像，與題幹要指認的音訊生成分屬不同模態。",
      C: "樞紐分析是結構化資料的統計彙整，產出的是表格與摘要數字，過程中沒有任何內容被生成出來。",
      D: "自動產生單元測試屬程式碼生成，輸出是文字形式的程式碼，而非聲音。",
    },
    topic: "L12201 生成式 AI 應用領域與常見工具",
    difficulty: "易",
    source: "generated",
  },
  {
    ...
    choiceExplanations: {
      A: "若兩者完全相同就不需要並存兩個名詞；差別在於是否仍允許以程式碼擴充功能。",
      C: "No code 為了讓不寫程式的人也能操作，刻意收斂了可調整的範圍；保留少量程式碼擴充空間的那一方，客製彈性通常反而較高。",
      D: "Low code 仍需設定資料結構、流程與介面，只是可再輔以少量程式碼；按一個按鈕就完成不是任何一方的實際樣貌。",
    },
    topic: "L12101 No Code / Low Code 的基本概念",
    difficulty: "中",
    source: "generated",
  },
  {
    ...
    choiceExplanations: {
      A: "自建省下的是 API 費用，但換來 GPU、維運與人力的長期投入；哪一邊較省取決於用量規模與團隊能力，不能一概而論。",
      C: "初期授權或訂閱費只是總體擁有成本的一部分，維運、擴充與資料合規的支出往往在上線之後才逐漸顯現。",
      D: "題幹問的是「應納入的關鍵評估構面」，而自建與商用 API 對資料落在哪裡的影響本就不同，資料隱私與安全恰恰不能略過。",
    },
    topic: "L12301 生成式 AI 導入評估",
    difficulty: "難",
    source: "generated",
  },
  {
    ...
    choiceExplanations: {
      A: "若導入後即保證正確，就沒有必要再分別評估檢索品質與答案品質；RAG 提高的是答案有依據的機會，來源本身有誤或檢索失敗時仍會錯。",
      C: "RAG 的重點是為生成提供可查證的依據，與調整取樣隨機性是兩回事；提高溫度反而讓答案更容易偏離檢索到的內容。",
      D: "RAG 是把檢索到的內容放進當次上下文，模型參數並未改變；也正因為知識留在外部知識庫，它才需要持續維護與更新。",
    },
    topic: "L12202 如何善用生成式 AI 工具",
    difficulty: "中",
    source: "generated",
  ...
  },
  {
    ...
    choiceExplanations: {
      A: "寫得更長更漂亮仍停留在指令文字這一層；題幹要問的範圍差異在於還要決定哪些資訊該進入模型視野、以及在 token 預算下如何取捨。",
      B: "溫度是生成時的取樣參數，不決定模型看得到哪些資訊，與脈絡的組成與管理不在同一個層面。",
      D: "若兩者相同，就無法解釋長任務中為何還得持續決定資訊要保留、摘要、檢索或丟棄——這些都不是把提示詞寫好就涵蓋得了的。",
    },
    topic: "L12202 如何善用生成式 AI 工具",
    difficulty: "中",
    source: "generated",
  ...
  },
  {
    ...
    choiceExplanations: {
      B: "少了成功條件與停止條件，迴圈就無從判斷任務何時算完成，也可能反覆重試而停不下來；效率不是省略這兩者的理由。",
      C: "高風險、低信心或權限不足正是應該暫停、交由人工決定的時機；在此仍自行執行，等於把最危險的情況交給最沒把握的判斷。",
      D: "一次給到最大權限違反最小權限原則；Agent 一旦被誤導或判斷失準，可造成的破壞範圍也同步被放大。",
    },
    topic: "L12202 如何善用生成式 AI 工具",
    difficulty: "中",
    source: "generated",
  ...
  },
  {
    ...
    choiceExplanations: {
      A: "固定 Workflow 的步驟與分支由程式事先寫定，走哪一條由程式判斷；題幹描述的是由一個 Agent 把控制權與狀態交給另一個 Agent。",
      B: "單一 Agent 是同一個模型在迴圈中自行選工具，從頭到尾沒有題幹所說的「交給另一個 Agent 接手」這個動作。",
      C: "平行型態是多個 Agent 同時探索、最後再綜合結果；題幹描述的是控制權依序轉交，同一時間仍只有一方在處理該請求。",
    },
    topic: "L12201 生成式 AI 應用領域與常見工具",
    difficulty: "中",
    source: "generated",
  ...
  },
  {
    ...
    choiceExplanations: {
      A: "把檢索到的外部內容一律當成可信任指令執行，正是題幹所述攻擊得以成立的原因，等於把防線交給攻擊者撰寫。",
      C: "注入利用的是模型不易區分「指令」與「待處理資料」這件事，只要把內容寫得像正當指示就可能奏效，與模型規模不是同一個維度。",
      D: "把工具權限開到最大並不會降低被注入的機率，只會讓注入一旦成功後可執行的破壞範圍更大。",
    },
    topic: "L12303 生成式 AI 風險管理",
    difficulty: "難",
    source: "generated",
  ...
  },
  {
    ...
    choiceExplanations: {
      B: "元件層的準確率再高，仍可能因為步驟選錯、工具用錯或結果沒真的落地而任務失敗；這正是題幹所說要從單一模型分數走向任務成功的理由。",
      C: "文筆流暢與內容正確是兩回事，讀起來順暢的答案同樣可能缺乏依據或引用錯誤，這在分層中屬答案層且不只看文筆。",
      D: "步驟、工具與參數選得對不對，決定了答案是怎麼來的；不看軌跡就分辨不出這次答對是穩定的做對，還是剛好猜中。",
    },
    topic: "L12301 生成式 AI 導入評估",
    difficulty: "中",
    source: "generated",
  ...
  },
  {
    ...
    choiceExplanations: {
      A: "讀起來是否通順屬於語言表面的品質，正是這個指標刻意不採信的部分——看似合理不等於使用者要辦的事真的辦成了。",
      B: "用了幾種工具只是過程的統計，多用不等於用得對；工具使用另有 Tool correctness 與 Trajectory efficiency 等指標衡量。",
      C: "字數多寡是輸出長度，與是否達成事先定義的成功條件沒有對應關係。",
    },
    topic: "L12301 生成式 AI 導入評估",
    difficulty: "中",
    source: "generated",
  ...
  },
  {
    ...
    choiceExplanations: {
      A: "只產生建議而不執行雖然最安全，但退款、部署這類動作終究得有人執行，全停在建議層會讓流程停滯；題幹問的是這類動作最適合放的層級，不是最保守的層級。",
      C: "長時間、多工具、低監督的高自主只適合可驗證、可停止且治理成熟的低風險情境；用在不可逆的退款或對外訊息上，出錯時已無從挽回。",
      D: "不分級等於讓高風險與低風險動作享有相同權限，正是過度代理權風險的來源，而代價由不可逆的後果承擔。",
    },
    topic: "L12303 生成式 AI 風險管理",
    difficulty: "難",
    source: "generated",
  ...
  },
  {
    ...
    choiceExplanations: {
      A: "由同一個模型產生答案、又自訂標準替自己打分，再把高分結果當成新的真實資料，等於拿自己的盲點驗證自己，分數再高也沒有外部依據，正是「禁止自我證明」要擋的做法。",
      B: "合成資料的分布來自生成模型，本就可能偏離真實世界；不控制比例也不檢查品質地反覆混入，正是模型能力與多樣性退化的成因。",
      D: "罕見、昂貴或危險的案例在真實世界難以取得，合成資料在邊界測試與紅隊演練上恰恰是有用的補充。",
    },
    topic: "L12302 生成式 AI 導入規劃",
    difficulty: "難",
    source: "generated",
  ...
  },
  {
    ...
    choiceExplanations: {
      B: "線上資料夾雜雜訊、誤用甚至惡意輸入，沒有審批就直接改動正式模型，錯誤會立即對所有使用者生效，也難以追溯是哪一批資料造成。",
      C: "不記錄版本就無法確認行為變化來自哪一次改動，出事時也回不到上一個可用版本，等於放棄了 rollback 這道保險。",
      D: "模型、提示、知識庫、工具或供應商任一端更新都可能改變系統行為，把上線當終點會讓這些變動在無人察覺的情況下累積。",
    },
    topic: "L12302 生成式 AI 導入規劃",
    difficulty: "中",
    source: "generated",
